...
fun DeviceListScreen(
    ...
    val groupedDevices = remember(uiState.devices, uiState.selectedGroupId) {
        buildDeviceSections(uiState.devices, uiState.selectedGroupId)
    }
    val availableGroups = remember(uiState.devices) {
        uiState.devices
            .flatMap { device -> device.groups }
            .distinctBy(DeviceGroup::id)
            .sortedBy { group -> group.name.ifBlank { "未分组" } }
    }
    val selectedGroupName = remember(availableGroups, uiState.selectedGroupId) {
...
private fun buildDeviceSections(devices: List<Device>, selectedGroupId: Int): List<DeviceSection> {
    if (selectedGroupId != 0) {
        val selectedDevices = devices.filter { device -> device.groups.any { it.id == selectedGroupId } }
        val title = selectedDevices.firstOrNull()
            ?.groups
